export default async function authRoutes(app) {
  ...
  app.post('/login', async (request, reply) => {
    ...
    const { data, error } = await request.supabase.auth.signInWithPassword({ email, password })
    if (error) return reply.code(401).send({ error: 'Email หรือ Password ไม่ถูกต้อง' })

    const { access_token, refresh_token } = data.session

    // เซ็ต cookie (httpOnly = JS อ่านไม่ได้ ปลอดภัยกว่า)
    reply
      .setCookie('access_token',  access_token,  { httpOnly: true, path: '/', maxAge: 60 * 60 * 8 })
      .setCookie('refresh_token', refresh_token, { httpOnly: true, path: '/', maxAge: 60 * 60 * 24 * 7 })
  ...
  app.get('/me', async (request, reply) => {
    ...
    if (error || !user) return reply.code(401).send({ error: 'Session หมดอายุ' })
    ...
    if (!appUser?.is_active) return reply.code(403).send({ error: 'บัญชีถูกระงับ' })
